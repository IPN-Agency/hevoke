...
/*---------------------------------------------------------------------- GET POST /SIGNUP --------------------------------------------*/
 exports.signup = function(req, res){
    ...
    if(req.method == "POST"){
       ...
       var opt_newsletter = post.opt_newsletter;
       if(opt_newsletter==undefined){ opt_newsletter="N"; }
       if(opt_newsletter=="on"){ opt_newsletter="Y"; }
       ...
       bcrypt.hash(password, 10, function(err, hash){
            if(err){
               ...
               var sql = "INSERT INTO UserArtist (user_id, user_name, artist_name, email, password, verif_code, opt_newsletter) VALUES ?"; console.log(sql);
               var values = [[uuid, username, username, email, hashed_password, verif_code, opt_newsletter]];
               var query = db.query(sql, [values], function(err, result) {
                           ...
                           */
                           ...
                           htmlString += '<link rel="preconnect" href="https://fonts.gstatic.com"><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;700&family=Kanit:ital,wght@0,400;0,800;0,900;1,800&display=swap" rel="stylesheet"> <script src="https://kit.fontawesome.com/ce061fb4d7.js" crossorigin="anonymous"></script>';
                           htmlString += '<style type="text/css" media="screen">';
                           htmlString += '[style*="Roboto"] { font-family: Arial, sans-serif; }';
                           htmlString += 'body { min-width:100% !important; width:100%!important; line-height: 1.5; -webkit-text-size-adjust:none; color: #303030; font-size: 16px; }';
                           htmlString += 'img { -ms-interpolation-mode: bicubic; /* Allow smoother rendering of resized image in Internet Explorer */ }';
                           htmlString += 'a { text-decoration: none; }';
                           htmlString += '</style>';
                           htmlString += '</head>';
                           htmlString += '<body>';
                           htmlString += '<div style="background-color: #070707; font-family: \'DM Sans\', sans-serif; color: #FFF; text-align: center; padding: 1em; font-weight: 300;">';
                           htmlString += '<div style="background-color: #070707; max-width: 950px; margin: 0 auto; padding: 1em; text-align: center; letter-spacing: -0.05em;">';
                           htmlString += '<div style="background-color: #070707; color: #FFF; font-family: \'Kanit\', sans-serif; font-weight: 400; font-size: 19px; margin-top: 2em"></div>';
                           htmlString += '<div style="background-color: #070707; font-size: 2.5em; line-height: 1em; margin-top: 35px; font-weight: 400;">Welcome on Hevoke!</div>';     
                           htmlString += '<div style="background-color: #070707; font-size: 1.2em; line-height: 1.25em; margin-top: 10px;">To activate your account use the button below:</div>';     
                           htmlString += '<a style="background-color: #484848; border: 0px; color: #FFF; font-size: 17px; font-weight: 400; text-decoration: none; padding: 0.75rem 2.2rem; margin-top: 20px; display: inline-block; width: auto; border-radius: 1px; text-transform: uppercase;" href='+link+'>Confirm</a>';
                           htmlString += '<div style="background-color: #070707; color:#E8E8E8; font-size: .9em; line-height: 15px; margin-top: 20px;">or copy and paste this url:</div>';   
                           htmlString += '<div style="background-color: #070707; color:#808080; font-size: .7em; line-height: 15px; margin-top: 5px; white-space: wrap"><i style="color: #AEAEAE;">'+link+'</i></div>';     
                           htmlString += '<div style="background-color: #070707; color:#E8E8E8; margin-top: 3em; padding: 2em 0 1em 0; border-top: 1px solid #383838;">';
                           htmlString += '<div style="background-color: #070707; color:#707070; font-size: .9em;">Connect with Hevoke on Instagram:  <a style="color: #FFF;" href="https://instagram.com/hevoke.com_">@hevoke.com_</a></div>';
                           htmlString += '</div>';
                           htmlString += '<div style="background-color: #070707; color:#707070; font-size: .8em; padding: .3em 0;">Need help? <a style="color: #FFF;" href="mailto:support@hevoke.com">support@hevoke.com</a></div>';   
                           htmlString += '<div style="background-color: #070707; color:#5C5C5C; font-size: .7em; padding: .3em 0;"><a style="color: #707070;" href="https://hevoke.com">hevoke.com</a></div>';
                           htmlString += '</div>';
                           htmlString += '</div>';
                           htmlString += '</body>';
                           htmlString += '</html>';
...
*/
                  ...
                  var htmlString='<html>';
                           ...
                           htmlString += '<div style="color:#ffffff; font-family: \'Kanit\', sans-serif; letter-spacing: 1px; font-weight: 400; font-size: 16px; margin-top: 5px;">Hevoke</div>';
                           htmlString += '<div style="font-size: 28px; line-height: 30px; letter-spacing: -1px; margin-top: 35px; font-weight: 500;">Reset your password</div>';     
...
exports.resetPassword = function(req, res){
   ...
   if(req.method == "POST"){ //IF REQ POST
      ...
      var sql = "SELECT email FROM TokenResetPassword WHERE token='"+token+"' LIMIT 1;";          
...
exports.setNewEmail = function(req, res){
   ...
   if(logged && req.method == "POST"){
         ...
         var sql = "SELECT email FROM UserArtist WHERE user_id='" + userLoggedId + "' LIMIT 1";
         console.log("get email of logged user: " + sql);
         var query = db.query(sql, function(err, result) {
            if(err){
               status = "ko";
            }
         ...
         console.log(timestamp);
         var timestamp_expiration = timestamp;
         var sql = "INSERT INTO TokenChangeEmail (user_id, email_new, token, exp_date) VALUES ('" + userLoggedId + "', '" + email_new + "', '" + token + "', '"+timestamp_expiration+"')";
         ...
         var query = db.query(sql, function(err, result) {
               if(err){
                  ...
                  var htmlString='<html>';
                           ...
                           /*htmlString += '<div style="line-height:30px; margin-top: 0px;"><img style="display:inline-block; line-height:0px;" src="https://webdevtrick.com/wp-content/uploads/logo-fb-1.png" width="60" height="60" alt="logo"></div>';*/
                           htmlString += '<div style="color:#ffffff; font-family: \'Kanit\', sans-serif; letter-spacing: 1px; font-weight: 400; font-size: 16px; margin-top: 5px;">Hevoke</div>';
                           htmlString += '<div style="font-size: 28px; line-height: 30px; letter-spacing: -1px; margin-top: 35px; font-weight: 500;">Confirm email address change</div>';     
                           htmlString += '<div style="font-size: 19px; line-height: 30px; margin-top: 10px;">If you want to change your Hevoke email to '+email_new+', click the button below:</div>';     
                  ...
                  transporter.sendMail(mailOptions, function(error, info){
                     if (error) {
                        ...
                        console.log('Email sent to ' + current_email + ' :' + info.response);
                        status = "ok";
                        message = message + "Email for verification has been sent to " + current_email;
...
exports.updateNewEmail = function(req, res){
   ...
   if(req.method == "GET"){
      ...
      var sql = "SELECT * FROM TokenChangeEmail WHERE token='"+url_code+"'LIMIT 1;";          
      db.query(sql, function(err, results){  

            if(results.length == 1){
               message = "Token is valid. We are about to update your email address.";
               ...
               db.query(sql, function(err, results){  
                  if(err){
                     ...
                     message = "Your account email has been set to " + new_email; console.log(message);
               ...
               message = "Token is not valid. Try one more time. If you need help send us a message to support@hevoke.com."; console.log(message);
               res.render('update-email.ejs',{status: false, message});
            }
...
function getMacroFromCategory(category){
   if(category=="video director" || category=="videographer" || category=="graphic designer"){  artist_macro = "V"; }
         ...
         arr_links.forEach(element => {
            ...
            db.query(sql, (err, result)=>{
               ...
               if(result[0].count_link == 0){
                  if(element.name=="instagram" || element.name=="facebook" || element.name=="twitter") { link_type = "connect"; }
...
//-------------------------------------------------------------------------------------------------------------------
//var upload = multer({ dest: "public/uploads" }).single("photo");  // If you do not want to use diskStorage then uncomment it 
...
exports.postEditImage = function(req, res){
  ...
  exports.postRemoveImage = function(req, res){
     var logged = check_logged(req.session.userLoggedId);
     if(!logged){ res.redirect("/login");  return; }
     var userLogged =  req.session.userLogged;
     var userLoggedId = req.session.userLoggedId;
     var status; var message
     const directoryPath= `public/uploads/${req.session.userLoggedId}`;
     console.log('directoryPath :>> ', directoryPath);

     if(deleteFolderRecursive(directoryPath) == false){
        status = "KO"; message = "not remove image"; 
        res.render("edit-profile-response.ejs", {logged, userLogged, status, message});
     }else{
        // SUCCESS, image successfully uploaded 
        status = "OK"; message = "";
