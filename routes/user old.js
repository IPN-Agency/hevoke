       ...
       bcrypt.hash(password, 10, function(err, hash){
            if(err){
               ...
               var query = db.query(sql, [values], function(err, result) {
                           ...
                           var link = "http://localhost:8080/account/verify?code="+verif_code;
                           ...
                           */
                           ...
                           htmlString += '<div style="font-size: 45px; line-height: 45px; margin-top: 20px; font-weight: 300;">Welcome on L26!</div>';     
                           htmlString += '<div style="font-size: 22px; line-height: 30px; margin-top: 10px;">To activate your account use the button below:</div>';     
                           ...
                           var mailOptions = {
                              from: 'J&R',
                              to: email,
                              subject: 'Welcome on L26, confirm your registration',
                              html: htmlString
                           };
   ...
   if(req.method == "POST"){
      ...
      var query = db.query(sql, function(err, result) {
         ...
         var timestamp_expiration = timestamp;
            var sql = "INSERT INTO ResetPassword (email, token, exp_date) VALUES ('" + email + "', '" + verif_code + "', '"+timestamp_expiration+"')";
            ...
            var query = db.query(sql, function(err, result) {
               if(err){
                  ...
                  var link = "http://localhost:8080/account/reset-password?code="+verif_code;
                  ...
                  */
                  ...
                  htmlString += '</head>';
                  htmlString += '<body>';
                  htmlString += '<div style="color:#949699; font-size: 18px;  line-height:30px; margin-top: 0px;">L26</div>'; 
                  htmlString += '<div style="color:#707070; font-size: 29px; line-height:30px; margin-top: 20px;">Ripristina la password</div>';     
                  ...
                  from: 'j and rick',
                  to: email,
                  subject: 'L26, la tua richiesta di ripristino password',
                  html: htmlString
                  };
                  ...
                  transporter.sendMail(mailOptions, function(error, info){
                     if (error) {
                        ...
                        console.log('Email sent to ' + email + ' :' + info.response);
                        status = "ok";
                        message = message + "L'email per il ripristino della password è stata inviata.";
...
exports.resetPassword = function(req, res){
   ...
   if(req.method == "POST"){ //IF REQ POST
            ...
            bcrypt.hash(password, 10, function(err, hash){
                  ...
                  db.query(sql, function(err, results){  
                     if(err){
                        ...
                        message = "La password è stata aggiornata."; console.log(message);
      ...
      var sql = "SELECT * FROM ResetPassword WHERE token='"+url_code+"'LIMIT 1;";          
      db.query(sql, function(err, results){  

            if(results.length == 1){
               message = "Il token è valido. Ora puoi impostare una nuova password.";
               console.log(message);
               res.render('reset-password.ejs', {from: "get", status: true, message});
            }
            else{
               message = "Il token non è valido. Prova ad effettuare una nuova richiesta di ripristino password.";
               console.log(message);
   ...
   if(logged && req.method == "POST"){
         ...
         var query = db.query(sql, function(err, result) {
               if(err){
                  ...
                  var link = "http://localhost:8080/account/update-email?token="+token;
