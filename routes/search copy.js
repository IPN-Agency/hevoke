...
exports.connections = function(req, res){
   ...
   var p2 = queryWrapper("SELECT * FROM UserFollows JOIN UserArtist ON UserFollows.id_sender=UserArtist.user_id WHERE id_receiver='"+userLoggedId+"'");
   ...
   .then(([rows_likes, rows_followers, rows_signalscollab]) => {
      ...
      var num_followers = rows_followers.length;
      var num_signalscollab = rows_signalscollab.length;

      res.render('myconnections.ejs', {logged, userLogged, menuActiveConnections:true, active: "dashboard", num_likes, num_followers, num_signalscollab});
   })
   .catch(err => {
      res.redirect('/home');
   })
...
exports.ajaxShowMoreResults = function(req,res){
 ...
 exports.allUsersFYPRadar = function(req, res){
   ...
   db.query(sql, function(err, userLoggedRadarsCategories){
         ...
         var p1 = queryWrapper("SELECT user_id, artist_name, artist_type, artist_macro, country, user_level, user_name FROM UserArtist WHERE UserArtist.user_id IN (SELECT UserRadar.user_id FROM UserRadar WHERE UserRadar.category_id='"+userLoggedMainCategory+"' AND UserRadar.user_id IN (SELECT user_id FROM UserArtist WHERE UserArtist.active !=0 AND artist_type IN ("+stringRadarsCategories+")))");
